...
def search_character(input_file_path: str(), character_dict: dict()):
    ...
    for line in df['翻刻文']:
        ...
        for i in (0, line_len):
            if i+1 < line_len:
                two = line[i] + line[i+1]
                if two not in character_dict.keys():
                    character_dict[two] = 1
                else:
                    character_count = character_dict[two]
                    character_count = character_count + 1
                    character_dict[two] = character_count
            else:
                break
...
def output_csv(character_dict: dict(), publish: str()):
    ...
    df.to_csv(os.path.join(output_path, publish + '_2words.csv'))
